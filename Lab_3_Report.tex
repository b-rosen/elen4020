...
Section \ref{sec:Algorithms} below discusses the created algorithms
and how they were implemented. Section \ref{sec:Node-Graph} details
how the node graph problem was solved, while Section \ref{sec:Results}
gives the results of testing the algorithms and finally, Section \ref{sec:Conclusions}
...
\cline{2-4} 
 & 100x100 (outA1) & 100x100 (outB1) & N/A\tabularnewline
\cline{2-4} 
 & 1000x500 (outA2) & 500x1000 (outB2) & N/A\tabularnewline
\cline{2-4} 
 & 1000x500 (outA3) & 500x1 (outB3) & N/A\tabularnewline
\hline 
\hline 
...
The results for the various input files are shown in Table \ref{tab:Results-for-Algorithm}.
Test files given by the lab is denoted by specifying the file name
in brackets under the matrix size. As is shown, the test files did
not run correctly. On inspecting the files, it was found that the
input file for the second matrix was not complete. In addition, the
...
directed graph as mentioned in Section \ref{sec:Node-Graph}.

Nevertheless, testing was successful with the authors' files. Although
...
represented as a matrix. The given lab files are not in the correct
